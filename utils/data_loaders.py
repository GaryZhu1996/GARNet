...
class ShapeNetDataset(torch.utils.data.dataset.Dataset):
    ...
    def __init__(self, dataset_type, file_list, n_views_rendering, transforms=None, rendom_for_reduce_view=None):
        ...
        # for view reduction
        self.rendom_for_reduce_view = rendom_for_reduce_view
    ...
    def get_datum(self, idx):
        ...
        # for view reduction
        elif self.dataset_type == DatasetType.TEST and self.rendom_for_reduce_view is not None:
            change_rendering_image_paths = [rendering_image_paths[i] for i in range(self.n_views_rendering)]
            selected_rendering_image_paths = [
                rendering_image_paths[i]
                for i in random.sample(range(len(change_rendering_image_paths)), self.rendom_for_reduce_view)
            ]
...
class ShapeNetDataLoader:
    ...
    # rendom_for_reduce_view for view reduction
    def get_dataset(self, dataset_type, n_views_rendering, transforms=None, rendom_for_reduce_view=None):
        files = []
        ...
        return ShapeNetDataset(dataset_type, files, n_views_rendering, transforms, rendom_for_reduce_view)
...
class ShapeNetChairRFCDataset(torch.utils.data.dataset.Dataset):
    ...
    def get_datum(self, idx):
        ...
        if len(rendering_image.shape) < 3:
            logging.error('It seems that there is something wrong with the image file %s' % rendering_image_path)
            sys.exit(2)
        ...
        return taxonomy_name, sample_name, np.asarray([rendering_image]), volume


# /////////////////////////////// = End of ShapeNetChairRFCDataset Class Definition = //////////////////////////////// #


class ShapeNetChairRFCDataLoader:
    def __init__(self, cfg):
        self.dataset_taxonomy = None
        self.rendering_image_path_template = cfg.DATASETS.SHAPENET_CHAIR_RFC.RENDERING_PATH
        self.volume_path_template = cfg.DATASETS.SHAPENET_CHAIR_RFC.VOXEL_PATH
        
        # Load all taxonomies of the dataset
        with open(cfg.DATASETS.SHAPENET_CHAIR_RFC.TAXONOMY_FILE_PATH, encoding='utf-8') as file:
            self.dataset_taxonomy = json.loads(file.read())
    ...
    def get_dataset(self, dataset_type, n_views_rendering, transforms=None):
        ...
        for taxonomy in self.dataset_taxonomy:
            ...
            if dataset_type == DatasetType.TRAIN:
                ...
                logging.error('ShapeNetChairRFC only for train.')
                sys.exit(2)
            
            files.extend(self.get_files_of_taxonomy(taxonomy_folder_name, samples))
        
        logging.info('Complete collecting files of the dataset. Total files: %d.' % (len(files)))
        return ShapeNetChairRFCDataset(files, transforms)
    ...
    def get_files_of_taxonomy(self, taxonomy_folder_name, samples):
        ...
        for sample_idx, sample_name in enumerate(samples):
            ...
            if not os.path.exists(volume_file_path):
                logging.warn('Ignore sample 03001627/%s since volume file not exists.' % (sample_name))
                continue

            # Get file list of rendering images
            rendering_image_file_path = self.rendering_image_path_template % (sample_name, random.randint(0, 59))
...
# ////////////////////////////// = End of ShapeNetHighResDataLoader Class Definition = ////////////////////////////// #
...
class Pix3dDataset(torch.utils.data.dataset.Dataset):
    ...
    def get_datum(self, idx):
        ...
        if rendering_image.shape[-1] == 4:
            # logging.warn('It seems the image file %s is 4-channel-image.' % rendering_image_path)
        ...
        #     volume = utils.binvox_rw.read_as_3d_array(f)
        #     volume = volume.data.astype(np.float32)
        #     volume = np.flip(volume.transpose(2, 1, 0), 0).copy()
            # img = utils.helpers.get_volume_views(volume)
            # cv2.imwrite(f'show/%d_img.png'%idx, img)
        volume = scipy.io.loadmat(volume_path)
...
class Pix3dDataLoader:
    ...
    def get_dataset(self, dataset_type, n_views_rendering, transforms=None, random_for_reduce_view=None):
        files = []
    ...
    def get_files_of_taxonomy(self, taxonomy_name, samples):
        ...
        for sample_idx, sample_name in enumerate(samples):
            ...
            # volume_file_path = self.volume_path_template % (taxonomy_name, model_name)
...
DATASET_LOADER_MAPPING = {
    ...
    'ShapeNetChairRFC': ShapeNetChairRFCDataLoader
}  # yapf: disable
