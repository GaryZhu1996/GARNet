    ...
    def __call__(self, rendering_images, bounding_box=None):
        ...
        for img_idx, img in enumerate(rendering_images):
            ...
            if bounding_box is not None:
                ...
                name = dt.now().isoformat()
                cv2.imwrite(f'show/%s-%s.png' % (name, img_idx),
                            img[y_top:y_bottom + 1, x_left:x_right + 1] * 255)
                ...
                # processed_image = np.pad(processed_image, ((40, 40), (40, 40), (0, 0)), mode='empty')
                # processed_image = cv2.resize(processed_image, (self.img_size_w, self.img_size_h))
                # Debug
                # from datetime import datetime as dt
                # name = dt.now().isoformat()
                # cv2.imwrite(f'show/%s-%s.png' % (name, img_idx), img * 255)
                # cv2.imwrite(f'show/%s-%s-1.png' % (name, img_idx), processed_image * 255)
                # cv2.imwrite(f'show/%s-%s-.png' % (name, img_idx), processed_image * 255)
            else:
...
class RandomCrop(object):
    ...
    def __call__(self, rendering_images, bounding_box=None):
        ...
        for img_idx, img in enumerate(rendering_images):
            ...
            # name = dt.now().isoformat()
            # cv2.imwrite(f'show/%s-%s.png' % (name, img_idx), img*255)
            # cv2.imwrite(f'show/%s-%s-2.png' % (name, img_idx), processed_image*255)
        return processed_images
